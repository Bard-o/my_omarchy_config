  ...
  for _, wallpaper_dir in ipairs(dirs) do
    local handle = io.popen(
      "find -L " .. ShellEscape(wallpaper_dir)
        .. " -maxdepth 1 -type f \\( -name '*.jpg' -o -name '*.jpeg' -o -name '*.png' -o -name '*.gif' -o -name '*.bmp' -o -name '*.webp' -o -name '*.mp4' -o -name '*.mkv' -o -name '*.webm' -o -name '*.avi' -o -name '*.mov' -o -name '*.wmv' -o -name '*.ogm' \\) 2>/dev/null | sort"
    )
    if handle then
