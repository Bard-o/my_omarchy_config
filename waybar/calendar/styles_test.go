...
func TestNewStyles(t *testing.T) {
	...
	t.Run("header style uses Bold and Accent color", func(t *testing.T) {
		theme := DefaultTheme()
		styles := NewStyles(theme)

		// The Header style should apply Accent color and Bold
		rendered := styles.Header.Render("Test")
		if !strings.Contains(rendered, "\x1b[") {
			t.Error("Header should contain ANSI escape sequences (styled)")
		}
		if !strings.Contains(stripANSI(rendered), "Test") {
			t.Error("Header should contain the text 'Test'")
		}
	})
}
