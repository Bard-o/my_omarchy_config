...
func TestView(t *testing.T) {
	...
	t.Run("view has correct line count for calendar layout", func(t *testing.T) {
		...
		// Expected: header(1) + blank(1) + daynames(1) + 6 grid rows = 9 lines minimum
		lines := strings.Split(strings.TrimRight(view, "\n"), "\n")
		// Strip ANSI codes from each line for counting meaningful lines
		nonEmptyLines := 0
		...
		if nonEmptyLines < 8 {
			t.Errorf("View should have at least 8 non-empty lines (header + daynames + 6 grid rows), got %d", nonEmptyLines)
		}
	})
